...
public class StringUtil {
	 ...
	 * @author liuding
	 */
	public static String getUnicodeString(String str, String unicode) {
		if (StringUtils.isEmpty(unicode) || StringUtils.isEmpty(str)) {
			return str;
		}
		try {
			return new String(str.getBytes("ISO-8859-1"), unicode);
	...
	public static String getRandLetter() {
		...
		code = String.valueOf(x).toUpperCase();
		return code;
	 ...
	 * @author liuding
	 */
	public static String substringAfter(String src, String find) {
		int index = src.indexOf(find);
		if (index < 0)
			return src;
		return src.substring(index + find.length(), src.length());
	}
	 ...
	 * @author liuding
	 */
	public static String substringBetween(String src, String left, String right) {
		int index1 = src.indexOf(left);
		if (index1 < 0)
			return src;
		src = src.substring(index1 + left.length(), src.length());
		int index2 = src.indexOf(right);
		if (index2 < 0)
			return src;
		return src.substring(0, index2);
	}
